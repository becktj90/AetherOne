/**
 * AetherSense mission-control dashboard
 *
       ...
       newWorker.addEventListener("statechange", () => {
         if (newWorker.state === "installed" && navigator.serviceWorker.controller) {
           ...
           const reload = window.confirm(
             "🎉 AetherSense just received a fresh update!\n\n" +
...
const VIEW_META = {
  dashboard: {
    title: "Overview",
    subtitle: "Mission-control overview of air quality, signal classification, and room context.",
  },
  environment: {
    title: "Atmosphere",
    subtitle: "Environmental telemetry, derived atmosphere metrics, and outdoor context.",
  },
  analysis: {
    title: "Signals",
    subtitle: "Signal classification, air index, presence cues, and confidence interpretation.",
  },
  history: {
    title: "Trends",
...
function officeOdorState(d) {
  ...
  const primary = currentPrimary(d, "No dominant signal");
...
function currentPrimary(d, fallback) {
  ...
  return num(d.airScore) <= 30 ? "Clean Air" : "No dominant signal";
}
...
function syncWeatherMapPosition(d) {
  ...
  if (weatherMarker) {
    ...
    weatherMarker.bindTooltip(`${escapeHtml(d.city || "AetherSense location")}<br>${lat.toFixed(4)}, ${lon.toFixed(4)}`);
  }
...
function roomSummary(d) {
  ...
  if (score < 30) return "Mostly fresh room with no dominant signal pattern.";
  ...
  for (let i = 1; i < ordered.length; i += 1) {
    ...
    if (currFarts > prevFarts) {
      ...
      pushEvent({
        ...
        detail: `Now tracking ${Math.round(currFarts)} today. Primary read: ${currentPrimary(curr, "No dominant signal")}.`,
      });
    }
...
function buildMissionReport(d) {
  ...
  lines.push(`Primary signal: ${currentPrimary(d, "No dominant signal")}`);
...
function buildOdorReport(d) {
  ...
  lines.push(`${currentPrimary(d, "No dominant signal")} · ${Math.round(num(d.primaryConf))}% confidence`);
  lines.push("");
  lines.push("--- SIGNAL MIX ---");
  lines.push(`Air Index: ${Math.round(vscProxyConfidence(d))}% sulfur proxy`);
...
function paranormalScienceFactors(d) {
  ...
  if (hasConfidentPrimary(d)) {
    factors.push(`The signal classifier currently leans ${d.primary} at ${Math.round(num(d.primaryConf))}%, so the spooky label is being colored by a real odor channel, not just random text.`);
  } else {
    factors.push(`The signal classifier does not see one dominant smell, so this reads more like diffuse room drift than one obvious source.`);
  }
...
function buildParanormalReport(d) {
  ...
  if (d.paranormalEntity || d.paranormalReport) {
    ...
    lines.push(`Current signal: ${currentPrimary(d, "No dominant signal class")}`);
    lines.push("");
...
function buildBroReport(d) {
  ...
  lines.push(`Primary signal: ${currentPrimary(d, "No dominant signal class")}`);
  lines.push(`Severity Level: ${smellTierLabel(num(d.tier))} (${num(d.tier)}/5)`);
...
function renderOdorCard(d) {
  ...
  if (title) title.textContent = currentPrimary(d, num(d.airScore) <= 30 ? "Clean-air pattern" : "No dominant signal");
  if (sub) {
...
function renderIntelDrawer(d) {
  ...
  $("intel-summary").textContent = `${summary} Current signal: ${currentPrimary(d, "No dominant signal")}.`;
...
function renderHero(d) {
  ...
  $("hero-brief-primary").textContent = currentPrimary(d, "No dominant signal");
...
function renderDerivedMetrics(d) {
  ...
  $("derived-primary").textContent = currentPrimary(d, "No dominant signal");
...
function renderAirIndexGauge(d) {
...
function renderFartCard(d) {
  ...
  $("fart-sub").textContent = `Primary signal: ${currentPrimary(d, "No dominant signal")}. dVOC is ${fmtSigned(d.dVoc, 2)} and room quality is ${Math.round(num(d.airScore))}/100.`;
  ...
  renderAirIndexGauge(d);
  setHeaderPill(
    "fart-badge",
    vsc >= 70 ? "Priority air event" : "Air index",
